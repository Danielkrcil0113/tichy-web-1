<script lang="ts">
  ...
  // Import s opraveným názvem (2x 'd') a správnou cestou
  ...
  let { form, property_type, onBack }: { form: FormShape; property_type: string; onBack: () => void } = $props();

  // Pomocná funkce pro bezpečné vytažení původních hodnot z props bez triggerování varování
  ...
  function nextStep() {
    if (validateStep(step) && step < totalSteps) step += 1;
  }
...
<!-- rounded-4xl opraveno z rounded-[2rem] -->
<div class="relative overflow-hidden rounded-4xl bg-white p-6 shadow-2xl shadow-slate-200/50 ring-1 ring-slate-100 sm:p-12">
  
  {#if submittedSuccess}
    <!-- SUCCESS STATE -->
    <div in:fade class="flex min-h-80 flex-col items-center justify-center rounded-4xl border border-emerald-200 bg-emerald-50 p-8 text-center">
      ...
      <h3 class="mt-6 text-2xl font-extrabold text-slate-900">Poptávka byla úspěšně odeslána</h3>
      <p class="mt-3 max-w-xl leading-7 text-slate-600">Děkujeme za důvěru. Vaše údaje jsme přijali a brzy se vám ozveme s dalším postupem.</p>
    </div>
  {:else}
    <!-- HLAVIČKA S TLAČÍTKEM ZPĚT -->
    <div class="mb-8 flex items-center justify-between border-b border-slate-100 pb-5">
      <div class="flex items-center gap-3">
        <span class="text-sm font-semibold text-slate-500 uppercase tracking-wide">Vybráno:</span>
      ...
      <button type="button" aria-label="Změnit typ nemovitosti" onclick={onBack} class="text-sm font-semibold text-slate-500 transition-colors hover:text-indigo-600">
        &larr; Změnit typ
      </button>
    </div>
      ...
      <div class="flex items-center justify-between">
        {#each Array.from({ length: totalSteps }, (_, index) => index + 1) as stepNumber (stepNumber)}
          <div class="flex flex-col items-center gap-2">
            <div class={`flex h-8 w-8 items-center justify-center rounded-full text-sm font-semibold transition-colors duration-300 ${step >= stepNumber ? 'bg-indigo-600 text-white' : 'bg-slate-100 text-slate-400'}`}>
              {stepNumber}
            </div>
          </div>
    ...
    <form method="POST" enctype="multipart/form-data" class="min-h-80" novalidate use:enhance={handleSubmit}>
      ...
      {#if step === 1}
        <div in:fly={{ x: 20, duration: 400, delay: 100 }} class="space-y-8">
          <div>
            ...
            <div class="grid grid-cols-1 gap-3 sm:grid-cols-2">
              {#each [
                ...
                <button
                  ...
                  class={`rounded-xl border-2 p-4 text-center text-sm font-semibold transition-all duration-200 ${
                    purpose === p.id
            ...
            <!-- Skrytý input pro odeslání s formulářem -->
      ...
      {#if step === 2}
        <div in:fly={{ x: 20, duration: 400, delay: 100 }} class="grid gap-5 md:grid-cols-2">
          ...
          <div class="md:col-span-2">
            <!-- Vyčleněná komponenta s chytrým našeptávačem -->
          ...
          <div>
            ...
            <input id="area_m2_visible" type="number" bind:value={area_m2} placeholder="Např. 68" class={`block w-full rounded-xl border-2 px-4 py-3.5 text-slate-900 transition-all hover:bg-slate-200/50 focus:border-indigo-500 focus:bg-white focus:ring-0 ${getError('area_m2') ? 'border-rose-300 bg-rose-50' : 'border-transparent bg-slate-100'}`} />
          ...
          <div>
            <label for="disposition_visible" class="mb-1.5 block text-sm font-bold text-slate-700">
              {property_type === 'ostatni' ? 'Typ' : 'Dispozice'} *
            </label>
            <select id="disposition_visible" bind:value={disposition} class={`block w-full rounded-xl border-2 px-4 py-3.5 text-slate-900 transition-all hover:bg-slate-200/50 focus:border-indigo-500 focus:bg-white focus:ring-0 ${getError('disposition') ? 'border-rose-300 bg-rose-50' : 'border-transparent bg-slate-100'}`}>
          ...
          <div>
            ...
            <select id="condition_visible" bind:value={condition} class="block w-full rounded-xl border-2 border-transparent bg-slate-100 px-4 py-3.5 text-slate-900 transition-all hover:bg-slate-200/50 focus:border-indigo-500 focus:bg-white focus:ring-0">
          ...
          {#if property_type === 'byt'}
            <div>
              <label for="elevator_visible" class="mb-1.5 block text-sm font-bold text-slate-700">Je v domě výtah? *</label>
              <select id="elevator_visible" bind:value={elevator} class={`block w-full rounded-xl border-2 px-4 py-3.5 text-slate-900 transition-all hover:bg-slate-200/50 focus:border-indigo-500 focus:bg-white focus:ring-0 ${getError('elevator') ? 'border-rose-300 bg-rose-50' : 'border-transparent bg-slate-100'}`}>
                <option value="">Vyberte možnost...</option>
                <option value="ano">Ano</option>
                <option value="ne">Ne</option>
                <option value="nevim">Nevím</option>
              </select>
              {#if getError('elevator')}<p class="mt-2 text-sm font-medium text-rose-500">{getError('elevator')}</p>{/if}
            </div>
          {/if}
        </div>
      {/if}
      ...
      {#if step === 3}
        <div in:fly={{ x: 20, duration: 400, delay: 100 }} class="grid gap-5 md:grid-cols-2">
          <div class="md:col-span-2">
            <label for="full_name_visible" class="mb-1.5 block text-sm font-bold text-slate-700">Vaše jméno a příjmení *</label>
            <input id="full_name_visible" bind:value={full_name} placeholder="Např. Jan Novák" class={`block w-full rounded-xl border-2 px-4 py-3.5 text-slate-900 transition-all hover:bg-slate-200/50 focus:border-indigo-500 focus:bg-white focus:ring-0 ${getError('full_name') ? 'border-rose-300 bg-rose-50' : 'border-transparent bg-slate-100'}`} />
          ...
          <div>
            ...
            <input id="email_visible" type="email" bind:value={email} placeholder="vas@email.cz" class={`block w-full rounded-xl border-2 px-4 py-3.5 text-slate-900 transition-all hover:bg-slate-200/50 focus:border-indigo-500 focus:bg-white focus:ring-0 ${getError('email') ? 'border-rose-300 bg-rose-50' : 'border-transparent bg-slate-100'}`} />
          ...
          <div>
            ...
            <input id="phone_visible" bind:value={phone} placeholder="+420" class={`block w-full rounded-xl border-2 px-4 py-3.5 text-slate-900 transition-all hover:bg-slate-200/50 focus:border-indigo-500 focus:bg-white focus:ring-0 ${getError('phone') ? 'border-rose-300 bg-rose-50' : 'border-transparent bg-slate-100'}`} />
      ...
      {#if step === 4}
        <div in:fly={{ x: 20, duration: 400, delay: 100 }} class="space-y-6">
          <div>
            <span class="mb-1.5 block text-sm font-bold text-slate-700">Fotografie nemovitosti (nepovinné, max 10)</span>
            ...
            {#if fileError}<p class="mt-2 text-sm font-medium text-rose-500">{fileError}</p>{/if}
              ...
              <div class="mt-4 grid grid-cols-3 gap-4 sm:grid-cols-4 md:grid-cols-5">
                {#each previewUrls as previewUrl, index (index)}
                  <div class="group relative aspect-square overflow-hidden rounded-lg border border-slate-200 bg-slate-100">
                    ...
                    <button type="button" onclick={() => removeFile(index)} class="absolute right-1.5 top-1.5 rounded-full bg-slate-900/70 p-1.5 text-white opacity-0 backdrop-blur-sm transition-opacity hover:bg-rose-600 group-hover:opacity-100" aria-label="Odstranit fotografii">
          ...
          <div>
            <label for="note_visible" class="mb-1.5 block text-sm font-bold text-slate-700">Chcete nám k nemovitosti ještě něco upřesnit?</label>
            <textarea id="note_visible" bind:value={note} rows="4" placeholder="Např. k bytu patří garáž, dům má novou střechu..." class="block w-full rounded-xl border-2 border-transparent bg-slate-100 px-4 py-3.5 text-slate-900 transition-all hover:bg-slate-200/50 focus:border-indigo-500 focus:bg-white focus:ring-0"></textarea>
          </div>

          <div class={`rounded-2xl bg-indigo-50/50 p-5 ${getError('consent') ? 'ring-1 ring-rose-300' : 'ring-1 ring-indigo-100'}`}>
            <div class="flex items-start gap-3">
              <div class="flex h-6 items-center">
                <input id="consent" name="consent" type="checkbox" bind:checked={consent} class="h-5 w-5 rounded border-indigo-200 text-indigo-600 focus:ring-indigo-600" />
              </div>
              <label for="consent" class="text-sm leading-6 text-slate-700">
                Rozumím a souhlasím se <a href={resolve('/gdpr')} class="font-semibold text-indigo-600 underline decoration-indigo-200 underline-offset-2 hover:text-indigo-800">zpracováním údajů</a> pro vypracování posouzení tržní ceny. *
              </label>
            </div>
      ...
      <div class="mt-12 flex items-center justify-between border-t border-slate-100 pt-6">
        <button type="button" aria-label="Vrátit se o krok zpět" onclick={prevStep} class={`flex items-center gap-2 rounded-xl px-4 py-3 text-sm font-bold text-slate-500 transition-all hover:bg-slate-100 hover:text-slate-900 ${step === 1 ? 'invisible' : 'visible'}`}>
        ...
        {#if step < totalSteps}
          <button type="button" aria-label="Pokračovat na další krok" onclick={nextStep} class="flex items-center gap-2 rounded-xl bg-slate-900 px-8 py-3.5 text-sm font-bold text-white shadow-lg shadow-slate-900/20 transition-all active:translate-y-0 hover:-translate-y-0.5 hover:bg-indigo-600 hover:shadow-indigo-600/25">
            Další krok
        ...
        {:else}
          <button type="submit" aria-label="Odeslat formulář k ocenění" disabled={isSubmitting} class="flex w-full items-center justify-center gap-2 rounded-xl bg-indigo-600 px-8 py-4 text-base font-bold text-white shadow-xl shadow-indigo-600/30 transition-all active:translate-y-0 hover:-translate-y-0.5 hover:bg-indigo-500 disabled:cursor-not-allowed disabled:opacity-70 sm:w-auto">
